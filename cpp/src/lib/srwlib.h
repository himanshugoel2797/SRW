...
/**
 * Optical Element:
 * Hyperboloid Mirror ("mirror: hyperboloid" type)
 */
struct SRWLStructOpticsMirrorHyperboloid { //TW19012014
	double p, q; /* distance [m] from first focus ('source') to mirror center, and from center to second focus ('image') */
	double angGraz; /* grazing angle [rad] at mirror center at perfect orientation */
	double radSag; /* sagital radius of curvature [m] at mirror center */
	SRWLOptMir baseMir; /* general information about the mirror */
 ...
 * @see ...
 */
EXP int CALL srwlCalcElecFieldSR(SRWLWfr* pWfr, SRWLPrtTrj* pTrj, SRWLMagFldC* pMagFld, double* precPar =0, int nPrecPar =0, double* arParGPU =0); //HG20072026 added arParGPU
 ...
 * @see ...
 */
EXP int CALL srwlCalcIntFromElecField(char* pInt, SRWLWfr* pWfr, char pol, char intType, char depType, double e, double x, double y, double* arMeth=0, void* pFldTrj=0, double* arParGPU=0); //OC18022024
 ...
 * @see ...
 */
EXP int CALL srwlPropagElecField(SRWLWfr* pWfr, SRWLOptC* pOpt, int nInt=0, char** arID=0, SRWLRadMesh* arIM=0, char** arI=0, double* arParGPU=0); //OC18022024
 ...
 * @see ...
 */
EXP int CALL srwlUtiFFT(char* pcData, char typeData, double* arMesh, int nMesh, int dir, double* arParGPU=0); //OC18022024
...
//#ifdef _OFFLOAD_GPU //OC18022024 (commented-out) //HG30112023
/**
 * Implements GPU related operations.
 * @param [in] op operation to be performed:
 * 			   0= Deinitialize GPU
 * 			   1= Initialize GPU
 * @param [in] pvGPU optional GPU utilization related parameters
 * @return	integer error (>0) or warnig (<0) code
 * @see ...
 */
EXP int CALL srwlUtiGPUProc(int op, double* arParGPU=0); //OC19022024
...
//#endif //OC18022024 (commented-out)
