...
//	{//OC21012024: Why wrapping? Consider revising
...
//	void RadPointModifierPortable(srTEXZ& EXZ, srTEFieldPtrs& EPtrs, void* pBuf=0) //HG01122023
	...
	void RadPointModifier_PropToWaistBeyondParax(srTEXZ& EXZ, srTEFieldPtrs& EPtrs, srTDriftPropBufVars* pBufVars) //OC10112019
	{
		...
		double x = -(LambQx*LambQx + LambQz*LambQz); //OC19022024 (eliminating function calls for GPU)

		if(TreatPath == 1) //add i*k*L
		{
			double radicalOnePlusSmall = (::fabs(x) > 0.01)? sqrt(1. + x) : 1. + x*(0.5 + x*(-0.125 + x*(0.0625 + x*(-0.0390625 + x*(0.02734375 + x*(-0.0205078125 + x*0.01611328125)))))); //OC19022024 (CGenMathMeth::radicalOnePlusSmall(x) inline)
			PhaseShift += kL*radicalOnePlusSmall; //OC19022024
			//PhaseShift += kL*(CGenMathMeth::radicalOnePlusSmall(Mi_ThetaXe2_p_ThetaZe2)); //OC19022024 (commented-out, eliminating function calls for GPU)
		}
		else
		{
			double radicalOnePlusSmallMinusOne = (::fabs(x) > 0.01)? sqrt(1. + x) - 1. : x*(0.5 + x*(-0.125 + x*(0.0625 + x*(-0.0390625 + x*(0.02734375 + x*(-0.0205078125 + x*0.01611328125)))))); //OC19022024 (CGenMathMeth::radicalOnePlusSmallMinusOne(x) inline)
			PhaseShift += kL*radicalOnePlusSmallMinusOne; //OC19022024
			//PhaseShift += kL*(CGenMathMeth::radicalOnePlusSmallMinusOne(Mi_ThetaXe2_p_ThetaZe2)); //OC19022024 (commented-out, eliminating function calls for GPU)
		}
